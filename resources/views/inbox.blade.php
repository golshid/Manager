...
<div class="container col-6">
    ...
    <div class="btn-group-lg d-flex" role="group">
        <button type="radio" style="background-color:rgba(52,138,167,0.4);" id="button1" class="btn m-1 flex-fill btn-secondary"
            data-toggle="collapse" data-target="#collapseExample1" aria-expanded="true"
            aria-controls="collapseExample1">Pending</button>
        <button type="radio" style="background-color:rgba(93,211,158,0.4);" id="button2" class="btn m-1 flex-fill btn-secondary"
            data-toggle="collapse" data-target="#collapseExample2" aria-expanded="false"
            aria-controls="collapseExample2">Approved</button>
        <button type="radio" style="background-color:rgba(188,231,132,0.4);" id="button3" class="btn m-1 flex-fill btn-secondary"
            data-toggle="collapse" data-target="#collapseExample3" aria-expanded="false"
            aria-controls="collapseExample3">Rejected</button>
    </div>
            ...
            <div class="container">
                <div class="card mt-2 wholecard2">
                    <div class="card-body d-flex flex-row">
                        <img style="border-radius:50px; img-thumbnail" src="storage/avatars/{{$r->user->avatar}}" alt="" width="30px" height="30px">&nbsp;&nbsp;
                        <div class=" ml-2">
                            
                            <div class="d-flex flex-column">
                                
                                <a class="mt-2" style="text-decoration:none;"
                            href="{{route('request.showAdmin',['id'=>$r->id])}}">
                                    <h5>{{$r->title}}</h5>
                                </a>
                            </div>
                        </div>
                    ...
                    <div class="card-footer">
                        <span>Requested on {{$r->created_at->toDateString()}} by <a href="{{route('profile.find',['id'=>$r->user_id])}}"> {{$r->user_name}}</a></span>
                        <a href="{{route('request.reject',['id'=>$r->id])}}" class="btn btn-sm float-right btn-danger">Reject</a>
                        <a href="{{route('request.accept',['id'=>$r->id])}}" class="btn btn-sm mr-2 float-right btn-success">Accept</a>
    ...
    <div class="collapse mt-3" id="collapseExample2">
        <div class="card text-white wholecard1 card-body" style="border-radius: 5px; box-shadow: 2px 2px 20px 5px #474144;">
            ...
            <div class="container">
                <div class="card mt-2 text-white wholecard2">
                    <div class="card-body d-flex flex-row">
                        ...
                        <div class="ml-2">
                            <div class="d-flex flex-column">
                                <a class="mt-2" style="text-decoration:none;" href="{{route('request.showAdmin',['id'=>$r->id])}}">
                                    <h5>{{ $r->title}}</h5>
                                </a>
                            </div>
                        </div>
...
@foreach ($requests as $r)
    <div class="card mt-3 wholecard2">
        <div class="card-body  text-white d-flex flex-row">
            ...
            <div class="ml-2">
                <div class="d-flex flex-column">
                    <a class="mt-2" style="text-decoration:none;" href="{{route('request.showUser',['id'=>$r->id])}}">
                        <h5>{{$r->title}}</h5>
                    </a>
                </div>
            </div>
